...
/// Returns the x displacement of arg direction.
int32_t bc_Direction_dx(bc_Direction arg);

/// Returns the y displacement of arg direction.
int32_t bc_Direction_dy(bc_Direction arg);

/// Whether arg direction is a diagonal one.
uint8_t bc_Direction_is_diagonal(bc_Direction arg);

/// Returns the direction opposite arg one, or Center if it's Center.
bc_Direction bc_Direction_opposite(bc_Direction arg);

/// Returns the direction 45 degrees to the left (counter-clockwise) of
/// arg one, or Center if it's Center.
bc_Direction bc_Direction_rotate_left(bc_Direction arg);

/// Returns the direction 45 degrees to the right (clockwise) of arg one,
/// or Center if it's Center.
bc_Direction bc_Direction_rotate_right(bc_Direction arg);
...
char* bc_Direction_to_json(bc_Direction arg);
...
/// 
void delete_bc_MapLocation(bc_MapLocation* arg);
/// The planet of the map location.
bc_Planet bc_MapLocation_planet_get(bc_MapLocation* arg);
/// The x coordinate of the map location.
int32_t bc_MapLocation_x_get(bc_MapLocation* arg);
/// The y coordinate of the map location.
int32_t bc_MapLocation_y_get(bc_MapLocation* arg);
/// The planet of the map location.
void bc_MapLocation_planet_set(bc_MapLocation* arg, bc_Planet planet);
/// The x coordinate of the map location.
void bc_MapLocation_x_set(bc_MapLocation* arg, int32_t x);
/// The y coordinate of the map location.
void bc_MapLocation_y_set(bc_MapLocation* arg, int32_t y);
/// Returns the location one square from arg one in the given direction.
bc_MapLocation* bc_MapLocation_add(bc_MapLocation* arg, bc_Direction direction);
/// Returns the location one square from arg one in the opposite direction.
bc_MapLocation* bc_MapLocation_subtract(bc_MapLocation* arg, bc_Direction direction);
/// Returns the location `multiple` squares from arg one in the given
/// direction.
bc_MapLocation* bc_MapLocation_add_multiple(bc_MapLocation* arg, bc_Direction direction, int32_t multiple);
/// Returns the location translated from arg location by `dx` in the x
/// direction and `dy` in the y direction.
bc_MapLocation* bc_MapLocation_translate(bc_MapLocation* arg, int32_t dx, int32_t dy);
/// Computes the square of the distance from arg location to the specified
/// location. If on different planets, returns the maximum integer.
uint32_t bc_MapLocation_distance_squared_to(bc_MapLocation* arg, bc_MapLocation* o);
/// Returns the Direction from arg location to the specified location.
/// If the locations are equal arg method returns Center.
/// 
///  * DifferentPlanet - The locations are on different planets.
bc_Direction bc_MapLocation_direction_to(bc_MapLocation* arg, bc_MapLocation* o);
/// 
/// Determines whether arg location is adjacent to the specified location,
...
uint8_t bc_MapLocation_is_adjacent_to(bc_MapLocation* arg, bc_MapLocation* o);
/// 
/// Whether arg location is within the distance squared range of the
/// specified location, inclusive. False for locations on different planets.
uint8_t bc_MapLocation_is_within_range(bc_MapLocation* arg, uint32_t range, bc_MapLocation* o);
/// Create a human-readable representation of a MapLocation
char* bc_MapLocation_debug(bc_MapLocation* arg);
/// Deep-copy a MapLocation
bc_MapLocation* bc_MapLocation_clone(bc_MapLocation* arg);
/// Compare two MapLocations for deep equality.
uint8_t bc_MapLocation_eq(bc_MapLocation* arg, bc_MapLocation* other);
...
char* bc_MapLocation_to_json(bc_MapLocation* arg);
...
/// 
void delete_bc_VecMapLocation(bc_VecMapLocation* arg);
/// Create a human-readable representation of a VecMapLocation
char* bc_VecMapLocation_debug(bc_VecMapLocation* arg);
/// Deep-copy a VecMapLocation
bc_VecMapLocation* bc_VecMapLocation_clone(bc_VecMapLocation* arg);
/// The length of the vector.
uintptr_t bc_VecMapLocation_len(bc_VecMapLocation* arg);
/// Copy an element out of the vector.
bc_MapLocation* bc_VecMapLocation_index(bc_VecMapLocation* arg, uintptr_t index);
/// An immutable list of i32 objects
typedef struct bc_Veci32 bc_Veci32;
/// An immutable list of i32 objects
bc_Veci32* new_bc_Veci32();
/// 
void delete_bc_Veci32(bc_Veci32* arg);
/// Create a human-readable representation of a Veci32
char* bc_Veci32_debug(bc_Veci32* arg);
/// Deep-copy a Veci32
bc_Veci32* bc_Veci32_clone(bc_Veci32* arg);
/// The length of the vector.
uintptr_t bc_Veci32_len(bc_Veci32* arg);
/// Copy an element out of the vector.
int32_t bc_Veci32_index(bc_Veci32* arg, uintptr_t index);
/// 
typedef struct bc_Location bc_Location;
/// 
bc_Location* new_bc_Location();
/// 
void delete_bc_Location(bc_Location* arg);
...
uint8_t bc_Location_is_on_map(bc_Location* arg);
/// True if and only if the location is on the map and on arg planet.
uint8_t bc_Location_is_on_planet(bc_Location* arg, bc_Planet planet);
/// The map location of the unit.
/// 
///  * UnitNotOnMap - The unit is in a garrison or in space, and does not
///    have a map location.
bc_MapLocation* bc_Location_map_location(bc_Location* arg);
/// Whether the unit is in a garrison.
uint8_t bc_Location_is_in_garrison(bc_Location* arg);
/// The structure whose garrison the unit is in.
/// 
///  * UnitNotInGarrison - the unit is not in a garrison.
uint16_t bc_Location_structure(bc_Location* arg);
/// Whether the unit is in space.
uint8_t bc_Location_is_in_space(bc_Location* arg);
/// Determines whether arg location is adjacent to the specified location,
/// including diagonally. Note that squares are not adjacent to themselves,
/// and squares on different planets are not adjacent to each other. Also,
/// nothing is adjacent to something not on a map.
uint8_t bc_Location_is_adjacent_to(bc_Location* arg, bc_Location* o);
/// Whether arg location is within the distance squared range of the
/// specified location, inclusive. False for locations on different planets.
/// Note that nothing is within the range of something not on the map.
uint8_t bc_Location_is_within_range(bc_Location* arg, uint32_t range, bc_Location* o);
/// Create a human-readable representation of a Location
char* bc_Location_debug(bc_Location* arg);
/// Deep-copy a Location
bc_Location* bc_Location_clone(bc_Location* arg);
/// Compare two Locations for deep equality.
uint8_t bc_Location_eq(bc_Location* arg, bc_Location* other);
/// Deserialize a Location from a JSON string
bc_Location* bc_Location_from_json(char* s);
/// Serialize a Location to a JSON string
char* bc_Location_to_json(bc_Location* arg);
/// 
...
char* bc_Team_to_json(bc_Team arg);
/// 
typedef struct bc_Player bc_Player;
/// 
bc_Player* new_bc_Player(bc_Team team, bc_Planet planet);
/// 
void delete_bc_Player(bc_Player* arg);
/// 
bc_Team bc_Player_team_get(bc_Player* arg);
/// 
bc_Planet bc_Player_planet_get(bc_Player* arg);
/// 
void bc_Player_team_set(bc_Player* arg, bc_Team team);
/// 
void bc_Player_planet_set(bc_Player* arg, bc_Planet planet);
/// Create a human-readable representation of a Player
char* bc_Player_debug(bc_Player* arg);
/// Deep-copy a Player
bc_Player* bc_Player_clone(bc_Player* arg);
/// Compare two Players for deep equality.
uint8_t bc_Player_eq(bc_Player* arg, bc_Player* other);
/// Deserialize a Player from a JSON string
bc_Player* bc_Player_from_json(char* s);
/// Serialize a Player to a JSON string
char* bc_Player_to_json(bc_Player* arg);
/// An immutable list of bc::unit::UnitID objects
typedef struct bc_VecUnitID bc_VecUnitID;
/// An immutable list of bc::unit::UnitID objects
bc_VecUnitID* new_bc_VecUnitID();
/// 
void delete_bc_VecUnitID(bc_VecUnitID* arg);
/// Create a human-readable representation of a VecUnitID
char* bc_VecUnitID_debug(bc_VecUnitID* arg);
/// Deep-copy a VecUnitID
bc_VecUnitID* bc_VecUnitID_clone(bc_VecUnitID* arg);
/// The length of the vector.
uintptr_t bc_VecUnitID_len(bc_VecUnitID* arg);
/// Copy an element out of the vector.
uint16_t bc_VecUnitID_index(bc_VecUnitID* arg, uintptr_t index);
...
/// 
void delete_bc_VecUnitType(bc_VecUnitType* arg);
/// Create a human-readable representation of a VecUnitType
char* bc_VecUnitType_debug(bc_VecUnitType* arg);
/// Deep-copy a VecUnitType
bc_VecUnitType* bc_VecUnitType_clone(bc_VecUnitType* arg);
/// The length of the vector.
uintptr_t bc_VecUnitType_len(bc_VecUnitType* arg);
/// Copy an element out of the vector.
bc_UnitType bc_VecUnitType_index(bc_VecUnitType* arg, uintptr_t index);
/// A single unit in the game and all its associated properties.
typedef struct bc_Unit bc_Unit;
/// A single unit in the game and all its associated properties.
bc_Unit* new_bc_Unit();
/// 
void delete_bc_Unit(bc_Unit* arg);
/// Create a human-readable representation of a Unit
char* bc_Unit_debug(bc_Unit* arg);
/// Deep-copy a Unit
bc_Unit* bc_Unit_clone(bc_Unit* arg);
/// Deserialize a Unit from a JSON string
bc_Unit* bc_Unit_from_json(char* s);
/// Serialize a Unit to a JSON string
char* bc_Unit_to_json(bc_Unit* arg);
/// Compare two Units for deep equality.
uint8_t bc_Unit_eq(bc_Unit* arg, bc_Unit* other);
/// The unique ID of a unit.
uint16_t bc_Unit_id(bc_Unit* arg);
/// The team the unit belongs to.
bc_Team bc_Unit_team(bc_Unit* arg);
/// The current research level.
uintptr_t bc_Unit_research_level(bc_Unit* arg);
/// The unit type.
bc_UnitType bc_Unit_unit_type(bc_Unit* arg);
/// The location of the unit.
bc_Location* bc_Unit_location(bc_Unit* arg);
/// The current health.
uint32_t bc_Unit_health(bc_Unit* arg);
/// The maximum health.
uint32_t bc_Unit_max_health(bc_Unit* arg);
/// The unit vision range.
uint32_t bc_Unit_vision_range(bc_Unit* arg);
/// The damage inflicted by the robot during a normal attack.
/// 
///  * InappropriateUnitType - the unit is not a robot.
int32_t bc_Unit_damage(bc_Unit* arg);
/// The attack range.
/// 
///  * InappropriateUnitType - the unit is not a robot.
uint32_t bc_Unit_attack_range(bc_Unit* arg);
/// The movement heat.
/// 
///  * InappropriateUnitType - the unit is not a robot.
uint32_t bc_Unit_movement_heat(bc_Unit* arg);
/// The attack heat.
/// 
///  * InappropriateUnitType - the unit is not a robot.
uint32_t bc_Unit_attack_heat(bc_Unit* arg);
/// The movement cooldown.
/// 
///  * InappropriateUnitType - the unit is not a robot.
uint32_t bc_Unit_movement_cooldown(bc_Unit* arg);
/// The attack cooldown.
/// 
///  * InappropriateUnitType - the unit is not a robot.
uint32_t bc_Unit_attack_cooldown(bc_Unit* arg);
/// Whether the active ability is unlocked.
/// 
///  * InappropriateUnitType - the unit is not a robot.
uint8_t bc_Unit_is_ability_unlocked(bc_Unit* arg);
/// The active ability heat.
/// 
///  * InappropriateUnitType - the unit is not a robot.
uint32_t bc_Unit_ability_heat(bc_Unit* arg);
/// The active ability cooldown.
/// 
///  * InappropriateUnitType - the unit is not a robot.
uint32_t bc_Unit_ability_cooldown(bc_Unit* arg);
/// The active ability range. Arg is the range in which: workers can replicate, knights can javelin, rangers can snipe, mages can blink, and healers can overcharge.
/// 
///  * InappropriateUnitType - the unit is not a robot.
uint32_t bc_Unit_ability_range(bc_Unit* arg);
/// Whether the worker has already acted (harveted, blueprinted, built, or repaired) arg round.
/// 
///  * InappropriateUnitType - the unit is not a worker.
uint8_t bc_Unit_worker_has_acted(bc_Unit* arg);
/// The health restored when building a structure.
/// 
///  * InappropriateUnitType - the unit is not a worker.
uint32_t bc_Unit_worker_build_health(bc_Unit* arg);
/// The health restored when repairing a structure.
/// 
///  * InappropriateUnitType - the unit is not a worker.
uint32_t bc_Unit_worker_repair_health(bc_Unit* arg);
/// The maximum amount of karbonite harvested from a deposit in one turn.
/// 
///  * InappropriateUnitType - the unit is not a worker.
uint32_t bc_Unit_worker_harvest_amount(bc_Unit* arg);
/// The amount of damage resisted by a knight when attacked.
/// 
///  * InappropriateUnitType - the unit is not a knight.
uint32_t bc_Unit_knight_defense(bc_Unit* arg);
/// The range within a ranger cannot attack.
/// 
///  * InappropriateUnitType - the unit is not a ranger.
uint32_t bc_Unit_ranger_cannot_attack_range(bc_Unit* arg);
/// The maximum countdown for ranger's snipe, which is the number of turns that must pass before the snipe is executed.
/// 
///  * InappropriateUnitType - the unit is not a ranger.
uint32_t bc_Unit_ranger_max_countdown(bc_Unit* arg);
/// Whether the ranger is sniping.
/// 
///  * InappropriateUnitType - the unit is not a ranger.
uint8_t bc_Unit_ranger_is_sniping(bc_Unit* arg);
/// The target location for ranger's snipe.
/// 
///  * InappropriateUnitType - the unit is not a ranger.
///  * NullValue - the ranger is not sniping.
bc_MapLocation* bc_Unit_ranger_target_location(bc_Unit* arg);
/// The countdown for ranger's snipe. Errors if the ranger is not sniping.
/// 
///  * InappropriateUnitType - the unit is not a ranger.
///  * NullValue - the ranger is not sniping.
uint32_t bc_Unit_ranger_countdown(bc_Unit* arg);
/// The amount of health passively restored to itself each round.
/// 
///  * InappropriateUnitType - the unit is not a healer.
uint32_t bc_Unit_healer_self_heal_amount(bc_Unit* arg);
/// Whether arg structure has been built.
/// 
///  * InappropriateUnitType - the unit is not a structure.
uint8_t bc_Unit_structure_is_built(bc_Unit* arg);
/// The max capacity of a structure.
/// 
///  * InappropriateUnitType - the unit is not a structure.
uintptr_t bc_Unit_structure_max_capacity(bc_Unit* arg);
/// Returns the units in the structure's garrison.
/// 
///  * InappropriateUnitType - the unit is not a structure.
bc_VecUnitID* bc_Unit_structure_garrison(bc_Unit* arg);
/// Whether the factory is currently producing a unit.
/// 
/// * InappropriateUnitType - the unit is not a factory.
uint8_t bc_Unit_is_factory_producing(bc_Unit* arg);
/// The unit type currently being produced by the factory.
/// 
///  * InappropriateUnitType - the unit is not a factory.
/// * NullValue - the factory is not producing.
bc_UnitType bc_Unit_factory_unit_type(bc_Unit* arg);
/// The number of rounds left to produce a robot in arg factory.
/// 
///  * InappropriateUnitType - the unit is not a factory.
///  * NullValue - the factory is not producing.
uint32_t bc_Unit_factory_rounds_left(bc_Unit* arg);
/// The maximum number of rounds left to produce a robot in arg factory.
/// 
///  * InappropriateUnitType - the unit is not a factory.
uint32_t bc_Unit_factory_max_rounds_left(bc_Unit* arg);
/// Whether the rocket has already been used.
/// 
///  * InappropriateUnitType - the unit is not a rocket.
uint8_t bc_Unit_rocket_is_used(bc_Unit* arg);
/// The damage a rocket deals to adjacent units upon landing.
/// 
///  * InappropriateUnitType - the unit is not a rocket.
int32_t bc_Unit_rocket_blast_damage(bc_Unit* arg);
...
/// 
///  * InappropriateUnitType - the unit is not a rocket.
uint32_t bc_Unit_rocket_travel_time_decrease(bc_Unit* arg);
/// An immutable list of bc::unit::Unit objects
typedef struct bc_VecUnit bc_VecUnit;
/// An immutable list of bc::unit::Unit objects
bc_VecUnit* new_bc_VecUnit();
/// 
void delete_bc_VecUnit(bc_VecUnit* arg);
/// Create a human-readable representation of a VecUnit
char* bc_VecUnit_debug(bc_VecUnit* arg);
/// Deep-copy a VecUnit
bc_VecUnit* bc_VecUnit_clone(bc_VecUnit* arg);
/// The length of the vector.
uintptr_t bc_VecUnit_len(bc_VecUnit* arg);
/// Copy an element out of the vector.
bc_Unit* bc_VecUnit_index(bc_VecUnit* arg, uintptr_t index);
/// The map for one of the planets in the Battlecode world. Arg information defines the terrain, dimensions, and initial units of the planet.
typedef struct bc_PlanetMap bc_PlanetMap;
/// The map for one of the planets in the Battlecode world. Arg information defines the terrain, dimensions, and initial units of the planet.
bc_PlanetMap* new_bc_PlanetMap();
/// 
void delete_bc_PlanetMap(bc_PlanetMap* arg);
/// The planet of the map.
bc_Planet bc_PlanetMap_planet_get(bc_PlanetMap* arg);
/// The height of arg map, in squares. Must be in the range [MAP_HEIGHT_MIN, MAP_HEIGHT_MAX], inclusive.
uintptr_t bc_PlanetMap_height_get(bc_PlanetMap* arg);
/// The height of arg map, in squares. Must be in the range [MAP_WIDTH_MIN, MAP_WIDTH_MAX], inclusive.
uintptr_t bc_PlanetMap_width_get(bc_PlanetMap* arg);
/// The initial units on the map. Each team starts with 1 to 3 Workers on Earth.
bc_VecUnit* bc_PlanetMap_initial_units_get(bc_PlanetMap* arg);
/// The planet of the map.
void bc_PlanetMap_planet_set(bc_PlanetMap* arg, bc_Planet planet);
/// The height of arg map, in squares. Must be in the range [MAP_HEIGHT_MIN, MAP_HEIGHT_MAX], inclusive.
void bc_PlanetMap_height_set(bc_PlanetMap* arg, uintptr_t height);
/// The height of arg map, in squares. Must be in the range [MAP_WIDTH_MIN, MAP_WIDTH_MAX], inclusive.
void bc_PlanetMap_width_set(bc_PlanetMap* arg, uintptr_t width);
/// The initial units on the map. Each team starts with 1 to 3 Workers on Earth.
void bc_PlanetMap_initial_units_set(bc_PlanetMap* arg, bc_VecUnit* initial_units);
/// Validates the map and checks some invariants are followed.
/// 
///  * InvalidMapObject - the planet map is invalid.
void bc_PlanetMap_validate(bc_PlanetMap* arg);
/// Whether a location is on the map.
uint8_t bc_PlanetMap_on_map(bc_PlanetMap* arg, bc_MapLocation* location);
/// 
...
uint8_t bc_PlanetMap_is_passable_terrain_at(bc_PlanetMap* arg, bc_MapLocation* location);
/// The amount of Karbonite initially deposited at the given location.
/// 
/// LocationOffMap - the location is off the map.
uint32_t bc_PlanetMap_initial_karbonite_at(bc_PlanetMap* arg, bc_MapLocation* location);
/// Deep-copy a PlanetMap
bc_PlanetMap* bc_PlanetMap_clone(bc_PlanetMap* arg);
...
char* bc_StartGameMessage_to_json(bc_StartGameMessage* arg);
/// 
typedef struct bc_TurnMessage bc_TurnMessage;
/// 
bc_TurnMessage* new_bc_TurnMessage();
/// 
void delete_bc_TurnMessage(bc_TurnMessage* arg);
...
char* bc_TurnMessage_to_json(bc_TurnMessage* arg);
/// 
typedef struct bc_StartTurnMessage bc_StartTurnMessage;
/// 
bc_StartTurnMessage* new_bc_StartTurnMessage();
/// 
void delete_bc_StartTurnMessage(bc_StartTurnMessage* arg);
/// 
int32_t bc_StartTurnMessage_time_left_ms_get(bc_StartTurnMessage* arg);
/// 
uint32_t bc_StartTurnMessage_round_get(bc_StartTurnMessage* arg);
/// 
void bc_StartTurnMessage_time_left_ms_set(bc_StartTurnMessage* arg, int32_t time_left_ms);
/// 
void bc_StartTurnMessage_round_set(bc_StartTurnMessage* arg, uint32_t round);
...
char* bc_StartTurnMessage_to_json(bc_StartTurnMessage* arg);
/// 
typedef struct bc_ViewerMessage bc_ViewerMessage;
/// 
bc_ViewerMessage* new_bc_ViewerMessage();
/// 
void delete_bc_ViewerMessage(bc_ViewerMessage* arg);
...
char* bc_ViewerMessage_to_json(bc_ViewerMessage* arg);
/// 
typedef struct bc_ViewerKeyframe bc_ViewerKeyframe;
/// 
bc_ViewerKeyframe* new_bc_ViewerKeyframe();
/// 
void delete_bc_ViewerKeyframe(bc_ViewerKeyframe* arg);
...
char* bc_ViewerKeyframe_to_json(bc_ViewerKeyframe* arg);
/// 
typedef struct bc_ErrorMessage bc_ErrorMessage;
/// 
bc_ErrorMessage* new_bc_ErrorMessage();
/// 
void delete_bc_ErrorMessage(bc_ErrorMessage* arg);
/// 
char* bc_ErrorMessage_error_get(bc_ErrorMessage* arg);
/// 
void bc_ErrorMessage_error_set(bc_ErrorMessage* arg, char* error);
...
char* bc_ErrorMessage_to_json(bc_ErrorMessage* arg);
/// Create a human-readable representation of a ErrorMessage
char* bc_ErrorMessage_debug(bc_ErrorMessage* arg);
/// 
...
/// 
void delete_bc_ReceivedMessaTurnMessage(bc_ReceivedMessaTurnMessage* arg);
...
char* bc_ReceivedMessaTurnMessage_to_json(bc_ReceivedMessaTurnMessage* arg);
/// Create a human-readable representation of a ReceivedMessaTurnMessage
char* bc_ReceivedMessaTurnMessage_debug(bc_ReceivedMessaTurnMessage* arg);
/// 
typedef struct bc_SentMessage bc_SentMessage;
/// 
bc_SentMessage* new_bc_SentMessage();
/// 
void delete_bc_SentMessage(bc_SentMessage* arg);
/// 
char* bc_SentMessage_client_id_get(bc_SentMessage* arg);
/// 
bc_TurnMessage* bc_SentMessage_turn_message_get(bc_SentMessage* arg);
/// 
void bc_SentMessage_client_id_set(bc_SentMessage* arg, char* client_id);
/// 
void bc_SentMessage_turn_message_set(bc_SentMessage* arg, bc_TurnMessage* turn_message);
...
char* bc_SentMessage_to_json(bc_SentMessage* arg);
/// Create a human-readable representation of a SentMessage
char* bc_SentMessage_debug(bc_SentMessage* arg);
/// 
typedef struct bc_TurnApplication bc_TurnApplication;
/// 
bc_TurnApplication* new_bc_TurnApplication();
/// 
void delete_bc_TurnApplication(bc_TurnApplication* arg);
/// 
bc_StartTurnMessage* bc_TurnApplication_start_turn_get(bc_TurnApplication* arg);
/// 
bc_ViewerMessage* bc_TurnApplication_viewer_get(bc_TurnApplication* arg);
/// 
void bc_TurnApplication_start_turn_set(bc_TurnApplication* arg, bc_StartTurnMessage* start_turn);
/// 
void bc_TurnApplication_viewer_set(bc_TurnApplication* arg, bc_ViewerMessage* viewer);
/// 
...
/// 
void delete_bc_InitialTurnApplication(bc_InitialTurnApplication* arg);
/// 
bc_StartTurnMessage* bc_InitialTurnApplication_start_turn_get(bc_InitialTurnApplication* arg);
/// 
bc_ViewerKeyframe* bc_InitialTurnApplication_viewer_get(bc_InitialTurnApplication* arg);
/// 
void bc_InitialTurnApplication_start_turn_set(bc_InitialTurnApplication* arg, bc_StartTurnMessage* start_turn);
/// 
void bc_InitialTurnApplication_viewer_set(bc_InitialTurnApplication* arg, bc_ViewerKeyframe* viewer);
...
/// 
void delete_bc_AsteroidStrike(bc_AsteroidStrike* arg);
/// 
uint32_t bc_AsteroidStrike_karbonite_get(bc_AsteroidStrike* arg);
/// 
bc_MapLocation* bc_AsteroidStrike_location_get(bc_AsteroidStrike* arg);
/// 
void bc_AsteroidStrike_karbonite_set(bc_AsteroidStrike* arg, uint32_t karbonite);
/// 
void bc_AsteroidStrike_location_set(bc_AsteroidStrike* arg, bc_MapLocation* location);
/// Deep-copy a AsteroidStrike
bc_AsteroidStrike* bc_AsteroidStrike_clone(bc_AsteroidStrike* arg);
/// Create a human-readable representation of a AsteroidStrike
char* bc_AsteroidStrike_debug(bc_AsteroidStrike* arg);
...
char* bc_AsteroidStrike_to_json(bc_AsteroidStrike* arg);
/// Compare two AsteroidStrikes for deep equality.
uint8_t bc_AsteroidStrike_eq(bc_AsteroidStrike* arg, bc_AsteroidStrike* other);
...
/// 
void delete_bc_AsteroidPattern(bc_AsteroidPattern* arg);
/// Validates the asteroid pattern.
/// 
///  * InvalidMapObject - the asteroid pattern is invalid.
void bc_AsteroidPattern_validate(bc_AsteroidPattern* arg);
/// Whether there is an asteroid strike at the given round.
uint8_t bc_AsteroidPattern_has_asteroid(bc_AsteroidPattern* arg, uint32_t round);
/// Get the asteroid strike at the given round.
/// 
///  * NullValue - There is no asteroid strike at arg round.
bc_AsteroidStrike* bc_AsteroidPattern_asteroid(bc_AsteroidPattern* arg, uint32_t round);
/// Deep-copy a AsteroidPattern
bc_AsteroidPattern* bc_AsteroidPattern_clone(bc_AsteroidPattern* arg);
/// Create a human-readable representation of a AsteroidPattern
char* bc_AsteroidPattern_debug(bc_AsteroidPattern* arg);
...
char* bc_AsteroidPattern_to_json(bc_AsteroidPattern* arg);
/// The orbit pattern that determines a rocket's flight duration. Arg pattern is a sinusoidal function y=a*sin(bx)+c.
typedef struct bc_OrbitPattern bc_OrbitPattern;
/// Construct a new orbit pattern. Arg pattern is a sinusoidal function y=a*sin(bx)+c, where the x-axis is the round number of takeoff and the the y-axis is the duration of flight to the nearest integer.
/// 
...
/// 
void delete_bc_OrbitPattern(bc_OrbitPattern* arg);
/// Amplitude of the orbit.
uint32_t bc_OrbitPattern_amplitude_get(bc_OrbitPattern* arg);
/// The period of the orbit.
uint32_t bc_OrbitPattern_period_get(bc_OrbitPattern* arg);
/// The center of the orbit.
uint32_t bc_OrbitPattern_center_get(bc_OrbitPattern* arg);
/// Amplitude of the orbit.
void bc_OrbitPattern_amplitude_set(bc_OrbitPattern* arg, uint32_t amplitude);
/// The period of the orbit.
void bc_OrbitPattern_period_set(bc_OrbitPattern* arg, uint32_t period);
/// The center of the orbit.
void bc_OrbitPattern_center_set(bc_OrbitPattern* arg, uint32_t center);
/// Validates the orbit pattern.
/// 
///  * InvalidMapObject - the orbit pattern is invalid.
void bc_OrbitPattern_validate(bc_OrbitPattern* arg);
/// Get the duration of flight if the rocket were to take off from either planet on the given round.
uint32_t bc_OrbitPattern_duration(bc_OrbitPattern* arg, uint32_t round);
...
char* bc_OrbitPattern_to_json(bc_OrbitPattern* arg);
/// The map defining the starting state for an entire game.
typedef struct bc_GameMap bc_GameMap;
/// The map defining the starting state for an entire game.
bc_GameMap* new_bc_GameMap();
/// 
void delete_bc_GameMap(bc_GameMap* arg);
/// Seed for random number generation.
uint16_t bc_GameMap_seed_get(bc_GameMap* arg);
/// Earth map.
bc_PlanetMap* bc_GameMap_earth_map_get(bc_GameMap* arg);
/// Mars map.
bc_PlanetMap* bc_GameMap_mars_map_get(bc_GameMap* arg);
/// The asteroid strike pattern on Mars.
bc_AsteroidPattern* bc_GameMap_asteroids_get(bc_GameMap* arg);
/// The orbit pattern that determines a rocket's flight duration.
bc_OrbitPattern* bc_GameMap_orbit_get(bc_GameMap* arg);
/// Seed for random number generation.
void bc_GameMap_seed_set(bc_GameMap* arg, uint16_t seed);
/// Earth map.
void bc_GameMap_earth_map_set(bc_GameMap* arg, bc_PlanetMap* earth_map);
/// Mars map.
void bc_GameMap_mars_map_set(bc_GameMap* arg, bc_PlanetMap* mars_map);
/// The asteroid strike pattern on Mars.
void bc_GameMap_asteroids_set(bc_GameMap* arg, bc_AsteroidPattern* asteroids);
/// The orbit pattern that determines a rocket's flight duration.
void bc_GameMap_orbit_set(bc_GameMap* arg, bc_OrbitPattern* orbit);
/// Validate the game map.
/// 
///  * InvalidMapObject - the game map is invalid.
void bc_GameMap_validate(bc_GameMap* arg);
/// 
bc_GameMap* bc_GameMap_test_map();
/// parse a .bc18t map file
bc_GameMap* bc_GameMap_parse_text_map(char* map);
/// Deep-copy a GameMap
bc_GameMap* bc_GameMap_clone(bc_GameMap* arg);
/// Deserialize a GameMap from a JSON string
bc_GameMap* bc_GameMap_from_json(char* s);
/// Serialize a GameMap to a JSON string
char* bc_GameMap_to_json(bc_GameMap* arg);
/// 
...
/// 
void delete_bc_ResearchInfo(bc_ResearchInfo* arg);
/// Returns the current level of the research branch.
uintptr_t bc_ResearchInfo_get_level(bc_ResearchInfo* arg, bc_UnitType branch);
/// Returns the research queue, where the front of the queue is at the beginning of the list.
bc_VecUnitType* bc_ResearchInfo_queue(bc_ResearchInfo* arg);
/// Whether there is a branch in the research queue.
uint8_t bc_ResearchInfo_has_next_in_queue(bc_ResearchInfo* arg);
...
bc_UnitType bc_ResearchInfo_next_in_queue(bc_ResearchInfo* arg);
/// Returns the number of rounds left until the upgrade at the front of the research queue is applied.
/// 
///  * NullValue - There is no branch to be researched.
uint32_t bc_ResearchInfo_rounds_left(bc_ResearchInfo* arg);
...
char* bc_ResearchInfo_to_json(bc_ResearchInfo* arg);
/// 
...
/// 
void delete_bc_RocketLanding(bc_RocketLanding* arg);
/// The ID of the rocket.
uint16_t bc_RocketLanding_rocket_id_get(bc_RocketLanding* arg);
/// The landing destination of the rocket.
bc_MapLocation* bc_RocketLanding_destination_get(bc_RocketLanding* arg);
/// The ID of the rocket.
void bc_RocketLanding_rocket_id_set(bc_RocketLanding* arg, uint16_t rocket_id);
/// The landing destination of the rocket.
void bc_RocketLanding_destination_set(bc_RocketLanding* arg, bc_MapLocation* destination);
/// Deep-copy a RocketLanding
bc_RocketLanding* bc_RocketLanding_clone(bc_RocketLanding* arg);
/// Create a human-readable representation of a RocketLanding
char* bc_RocketLanding_debug(bc_RocketLanding* arg);
...
char* bc_RocketLanding_to_json(bc_RocketLanding* arg);
/// Compare two RocketLandings for deep equality.
uint8_t bc_RocketLanding_eq(bc_RocketLanding* arg, bc_RocketLanding* other);
...
/// 
void delete_bc_VecRocketLanding(bc_VecRocketLanding* arg);
/// Create a human-readable representation of a VecRocketLanding
char* bc_VecRocketLanding_debug(bc_VecRocketLanding* arg);
/// Deep-copy a VecRocketLanding
bc_VecRocketLanding* bc_VecRocketLanding_clone(bc_VecRocketLanding* arg);
/// The length of the vector.
uintptr_t bc_VecRocketLanding_len(bc_VecRocketLanding* arg);
/// Copy an element out of the vector.
bc_RocketLanding* bc_VecRocketLanding_index(bc_VecRocketLanding* arg, uintptr_t index);
/// 
...
/// 
void delete_bc_RocketLandingInfo(bc_RocketLandingInfo* arg);
/// Get the rocket landings on arg round.
bc_VecRocketLanding* bc_RocketLandingInfo_landings_on(bc_RocketLandingInfo* arg, uint32_t round);
/// Deep-copy a RocketLandingInfo
bc_RocketLandingInfo* bc_RocketLandingInfo_clone(bc_RocketLandingInfo* arg);
/// Create a human-readable representation of a RocketLandingInfo
char* bc_RocketLandingInfo_debug(bc_RocketLandingInfo* arg);
...
char* bc_RocketLandingInfo_to_json(bc_RocketLandingInfo* arg);
/// Compare two RocketLandingInfos for deep equality.
uint8_t bc_RocketLandingInfo_eq(bc_RocketLandingInfo* arg, bc_RocketLandingInfo* other);
/// 
...
/// 
void delete_bc_GameController(bc_GameController* arg);
/// Send the moves from the current turn and wait for the next turn.
void bc_GameController_next_turn(bc_GameController* arg);
/// Get the time left at the start of arg player's turn, in milliseconds.
int32_t bc_GameController_get_time_left_ms(bc_GameController* arg);
...
uint32_t bc_GameController_round(bc_GameController* arg);
/// The current planet.
bc_Planet bc_GameController_planet(bc_GameController* arg);
/// The team whose turn it is.
bc_Team bc_GameController_team(bc_GameController* arg);
...
bc_PlanetMap* bc_GameController_starting_map(bc_GameController* arg, bc_Planet planet);
/// The karbonite in the team's resource pool.
uint32_t bc_GameController_karbonite(bc_GameController* arg);
/// The single unit with arg ID. Use arg method to get detailed statistics on a unit - heat, cooldowns, and properties of special abilities like units garrisoned in a rocket.
/// 
/// * NoSuchUnit - the unit does not exist (inside the vision range).
bc_Unit* bc_GameController_unit(bc_GameController* arg, uint16_t id);
/// All the units within the vision range, in no particular order. Does not include units in space.
bc_VecUnit* bc_GameController_units(bc_GameController* arg);
/// All the units on your team. Does not include units in space.
bc_VecUnit* bc_GameController_my_units(bc_GameController* arg);
/// All the units of arg team that are in space. You cannot see units on the other team that are in space.
bc_VecUnit* bc_GameController_units_in_space(bc_GameController* arg);
/// The karbonite at the given location.
/// 
/// * LocationOffMap - the location is off the map.
/// * LocationNotVisible - the location is outside the vision range.
uint32_t bc_GameController_karbonite_at(bc_GameController* arg, bc_MapLocation* location);
/// Returns an array of all locations within a certain radius squared of arg location that are on the map.
/// 
/// The locations are ordered first by the x-coordinate, then the y-coordinate. The radius squared is inclusive.
bc_VecMapLocation* bc_GameController_all_locations_within(bc_GameController* arg, bc_MapLocation* location, uint32_t radius_squared);
/// Whether the location is on the map and within the vision range.
uint8_t bc_GameController_can_sense_location(bc_GameController* arg, bc_MapLocation* location);
/// Whether there is a unit with arg ID within the vision range.
uint8_t bc_GameController_can_sense_unit(bc_GameController* arg, uint16_t id);
...
bc_VecUnit* bc_GameController_sense_nearby_units(bc_GameController* arg, bc_MapLocation* location, uint32_t radius);
...
bc_VecUnit* bc_GameController_sense_nearby_units_by_team(bc_GameController* arg, bc_MapLocation* location, uint32_t radius, bc_Team team);
...
bc_VecUnit* bc_GameController_sense_nearby_units_by_type(bc_GameController* arg, bc_MapLocation* location, uint32_t radius, bc_UnitType unit_type);
/// Whether there is a visible unit at a location.
uint8_t bc_GameController_has_unit_at_location(bc_GameController* arg, bc_MapLocation* location);
/// The unit at the location, if it exists.
/// 
/// * LocationOffMap - the location is off the map.
/// * LocationNotVisible - the location is outside the vision range.
bc_Unit* bc_GameController_sense_unit_at_location(bc_GameController* arg, bc_MapLocation* location);
/// The asteroid strike pattern on Mars.
bc_AsteroidPattern* bc_GameController_asteroid_pattern(bc_GameController* arg);
/// The orbit pattern that determines a rocket's flight duration.
bc_OrbitPattern* bc_GameController_orbit_pattern(bc_GameController* arg);
/// The current duration of flight if a rocket were to be launched arg round. Does not take into account any research done on rockets.
uint32_t bc_GameController_current_duration_of_flight(bc_GameController* arg);
/// Gets a read-only version of arg planet's team array. If the given planet is different from the planet of the player, reads the version of the planet's team array from COMMUNICATION_DELAY rounds prior.
bc_Veci32* bc_GameController_get_team_array(bc_GameController* arg, bc_Planet planet);
/// Writes the value at the index of arg planet's team array.
/// 
/// * ArrayOutOfBounds - the index of the array is out of bounds. It must be within [0, COMMUNICATION_ARRAY_LENGTH).
void bc_GameController_write_team_array(bc_GameController* arg, uintptr_t index, int32_t value);
...
void bc_GameController_disintegrate_unit(bc_GameController* arg, uint16_t unit_id);
/// Whether the location is clear for a unit to occupy, either by movement or by construction.
/// 
/// * LocationOffMap - the location is off the map.
/// * LocationNotVisible - the location is outside the vision range.
uint8_t bc_GameController_is_occupiable(bc_GameController* arg, bc_MapLocation* location);
...
uint8_t bc_GameController_can_move(bc_GameController* arg, uint16_t robot_id, bc_Direction direction);
/// Whether the robot is ready to move. Tests whether the robot's attack heat is sufficiently low.
uint8_t bc_GameController_is_move_ready(bc_GameController* arg, uint16_t robot_id);
...
void bc_GameController_move_robot(bc_GameController* arg, uint16_t robot_id, bc_Direction direction);
...
uint8_t bc_GameController_can_attack(bc_GameController* arg, uint16_t robot_id, uint16_t target_unit_id);
...
uint8_t bc_GameController_is_attack_ready(bc_GameController* arg, uint16_t robot_id);
...
void bc_GameController_attack(bc_GameController* arg, uint16_t robot_id, uint16_t target_unit_id);
/// The research info of the current team, including what branch is currently being researched, the number of rounds left.
bc_ResearchInfo* bc_GameController_research_info(bc_GameController* arg);
/// Resets the research queue to be empty. Returns true if the queue was not empty before, and false otherwise.
uint8_t bc_GameController_reset_research(bc_GameController* arg);
...
uint8_t bc_GameController_queue_research(bc_GameController* arg, bc_UnitType branch);
/// Whether the worker is ready to harvest, and the given direction contains karbonite to harvest. The worker cannot already have performed an action arg round.
uint8_t bc_GameController_can_harvest(bc_GameController* arg, uint16_t worker_id, bc_Direction direction);
...
/// * InappropriateUnitType - the unit is not a worker.
/// * Overheated - the worker has already performed an action arg turn.
...
void bc_GameController_harvest(bc_GameController* arg, uint16_t worker_id, bc_Direction direction);
/// Whether the worker can blueprint a unit of the given type. The worker can only blueprint factories, and rockets if Rocketry has been researched. The team must have sufficient karbonite in its resource pool. The worker cannot already have performed an action arg round.
uint8_t bc_GameController_can_blueprint(bc_GameController* arg, uint16_t worker_id, bc_UnitType unit_type, bc_Direction direction);
...
void bc_GameController_blueprint(bc_GameController* arg, uint16_t worker_id, bc_UnitType structure_type, bc_Direction direction);
/// Whether the worker can build a blueprint with the given ID. The worker and the blueprint must be adjacent to each other. The worker cannot already have performed an action arg round.
uint8_t bc_GameController_can_build(bc_GameController* arg, uint16_t worker_id, uint16_t blueprint_id);
...
void bc_GameController_build(bc_GameController* arg, uint16_t worker_id, uint16_t blueprint_id);
...
uint8_t bc_GameController_can_repair(bc_GameController* arg, uint16_t worker_id, uint16_t structure_id);
/// Commands the worker to repair a structure, repleneshing health to it. Arg can only be done to structures which have been fully built.
/// 
/// * NoSuchUnit - either unit does not exist (within the vision range).
/// * TeamNotAllowed - either unit is not on the current player's team.
/// * UnitNotOnMap - the worker is not on the map.
...
void bc_GameController_repair(bc_GameController* arg, uint16_t worker_id, uint16_t structure_id);
...
uint8_t bc_GameController_can_replicate(bc_GameController* arg, uint16_t worker_id, bc_Direction direction);
...
/// * LocationNotVisible - the location is outside the vision range.
/// * LocationNotEmpty - the location in the target direction is already occupied.
void bc_GameController_replicate(bc_GameController* arg, uint16_t worker_id, bc_Direction direction);
...
uint8_t bc_GameController_can_javelin(bc_GameController* arg, uint16_t knight_id, uint16_t target_unit_id);
/// Whether the knight is ready to javelin. Tests whether the knight's ability heat is sufficiently low.
uint8_t bc_GameController_is_javelin_ready(bc_GameController* arg, uint16_t knight_id);
...
void bc_GameController_javelin(bc_GameController* arg, uint16_t knight_id, uint16_t target_unit_id);
...
uint8_t bc_GameController_can_begin_snipe(bc_GameController* arg, uint16_t ranger_id, bc_MapLocation* location);
/// Whether the ranger is ready to begin snipe. Tests whether the ranger's ability heat is sufficiently low.
uint8_t bc_GameController_is_begin_snipe_ready(bc_GameController* arg, uint16_t ranger_id);
...
void bc_GameController_begin_snipe(bc_GameController* arg, uint16_t ranger_id, bc_MapLocation* location);
...
uint8_t bc_GameController_can_blink(bc_GameController* arg, uint16_t mage_id, bc_MapLocation* location);
/// Whether the mage is ready to blink. Tests whether the mage's ability heat is sufficiently low.
uint8_t bc_GameController_is_blink_ready(bc_GameController* arg, uint16_t mage_id);
...
/// * LocationOffMap - the target location is not on arg planet's map.
...
void bc_GameController_blink(bc_GameController* arg, uint16_t mage_id, bc_MapLocation* location);
...
uint8_t bc_GameController_can_heal(bc_GameController* arg, uint16_t healer_id, uint16_t target_robot_id);
/// Whether the healer is ready to heal. Tests whether the healer's attack heat is sufficiently low.
uint8_t bc_GameController_is_heal_ready(bc_GameController* arg, uint16_t healer_id);
...
void bc_GameController_heal(bc_GameController* arg, uint16_t healer_id, uint16_t target_robot_id);
...
uint8_t bc_GameController_can_overcharge(bc_GameController* arg, uint16_t healer_id, uint16_t target_robot_id);
/// Whether the healer is ready to overcharge. Tests whether the healer's ability heat is sufficiently low.
uint8_t bc_GameController_is_overcharge_ready(bc_GameController* arg, uint16_t healer_id);
...
void bc_GameController_overcharge(bc_GameController* arg, uint16_t healer_id, uint16_t target_robot_id);
...
uint8_t bc_GameController_can_load(bc_GameController* arg, uint16_t structure_id, uint16_t robot_id);
...
void bc_GameController_load(bc_GameController* arg, uint16_t structure_id, uint16_t robot_id);
...
uint8_t bc_GameController_can_unload(bc_GameController* arg, uint16_t structure_id, bc_Direction direction);
...
void bc_GameController_unload(bc_GameController* arg, uint16_t structure_id, bc_Direction direction);
...
uint8_t bc_GameController_can_produce_robot(bc_GameController* arg, uint16_t factory_id, bc_UnitType robot_type);
...
void bc_GameController_produce_robot(bc_GameController* arg, uint16_t factory_id, bc_UnitType robot_type);
/// The landing rounds and locations of rockets in space that belong to the current team.
bc_RocketLandingInfo* bc_GameController_rocket_landings(bc_GameController* arg);
...
uint8_t bc_GameController_can_launch_rocket(bc_GameController* arg, uint16_t rocket_id, bc_MapLocation* destination);
...
void bc_GameController_launch_rocket(bc_GameController* arg, uint16_t rocket_id, bc_MapLocation* location);
/// 
bc_GameController* bc_GameController_new_manager(bc_GameMap* map);
/// 
bc_StartGameMessage* bc_GameController_start_game(bc_GameController* arg, bc_Player* player);
/// 
bc_TurnApplication* bc_GameController_apply_turn(bc_GameController* arg, bc_TurnMessage* turn, int32_t time_left_ms);
/// 
bc_InitialTurnApplication* bc_GameController_initial_start_turn_message(bc_GameController* arg, int32_t time_left_ms);
/// 
uint8_t bc_GameController_is_over(bc_GameController* arg);
/// 
bc_Team bc_GameController_winning_team(bc_GameController* arg);
/// 
char* bc_GameController_manager_viewer_message(bc_GameController* arg);
/// 
void bc_GameController_print_game_ansi(bc_GameController* arg);
/// 
uint32_t bc_GameController_manager_karbonite(bc_GameController* arg, bc_Team team);
#ifdef __cplusplus
}
#endif
#endif // bc_h_
